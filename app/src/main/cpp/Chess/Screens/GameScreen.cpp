...
void Update(float dt)
{
	...
	else
	{
		...
		{
			Vector2 boardMousePos = {(mousePos.x - boardX) / boardSquareSize, (mousePos.y - boardY) / boardSquareSize};
			if (CheckCollisionPointRec(mousePos, {GetScreenWidth() - 200.0f, 0.0f, 200.0f, 200.0f})) // Save button
			{
				SaveBoard(board);
				saveBanner.Start();
			}
	...
	if (IsMouseButtonReleased(MOUSE_BUTTON_LEFT))
	{
		if (CheckCollisionPointRec(mousePos, {0, 0, 200, 200})) // Go back to main menu
			ScreenManager::ChangeScreen(MenuScreen::GetScreen());
	}

    if (saveBanner.IsDone())
        saveBanner = BannerAnimation(4.0f, "SAVED", {(float)GetScreenWidth() + 100.0f, 150.0f}, {-100.0f, 150.0f}, 35, WHITE, RED);
...
void Render()
{
    ...
    if (IsInCheck(board, PieceColor::White))
    {
		...
		{
			int text_length = MeasureText("Checkmate", 50);
            DrawText("Checkmate", GetScreenWidth() / 2 - text_length / 2, boardY - 300, 50, {234, 234, 234, 255});
		}
        else
		{
			int text_length = MeasureText("Check", 50);
			DrawText("Check", GetScreenWidth() / 2 - text_length / 2, boardY - 300, 50, {234, 234, 234, 255});
		}
    }
    else
    {
        if (board.whiteLegalMoves.size() == 0)
		{
			int text_length = MeasureText("Stalemate", 50);
			DrawText("Stalemate", GetScreenWidth() / 2 - text_length / 2, boardY - 300, 50, {234, 234, 234, 255});
		}
    }
    if (IsInCheck(board, PieceColor::Black))
    {
		...
		{
			int text_length = MeasureText("Checkmate", 50);
			DrawText("Checkmate", GetScreenWidth() / 2 - text_length / 2, boardY - 300, 50, {112, 112, 112, 255});
		}
        else
		{
			int text_length = MeasureText("Check", 50);
			DrawText("Check", GetScreenWidth() / 2 - text_length / 2, boardY - 300, 50, {112, 112, 112, 255});
		}
    }
    else
    {
	...
	const int point_x = 30;
	const int point_y = 100;
	const int arrow_height = 16;
	DrawRectangle(point_x + 10, point_y - arrow_height / 2, 95, arrow_height, WHITE);
	DrawRectanglePro((Rectangle){point_x, point_y, 55, arrow_height}, (Vector2){0, 0}, -45.0f, WHITE);
	DrawRectanglePro((Rectangle){point_x, point_y, 55, arrow_height}, (Vector2){0, arrow_height}, 45.0f, WHITE);
	// }
    
    DrawTextureEx(saveIconTexture, {GetScreenWidth() - 105.0f, 62.5f}, 0.0f, 1.5f, WHITE);
...
void OnResize(int width, int height)
{
    ...
    saveBanner = BannerAnimation(4.0f, "SAVED", {(float)width + 100.0f, 150.0f}, {-100.0f, 150.0f}, 35, WHITE, RED);

	settingsLayer.Resize(800, height);

	settingsAnim.RemoveKey(SETTING_X_ANIMATION);
	settingsAnim.AddKey(SETTING_X_ANIMATION, width, width - 800.0f);
}
