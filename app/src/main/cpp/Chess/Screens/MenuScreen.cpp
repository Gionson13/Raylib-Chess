...
void Load()
{
	fileDialogState = InitGuiWindowFileDialog("Saves/");
	settings = false;
...
static void RenderButton(const std::string& text, Rectangle rec)
{
	...
	int length = MeasureText(text.c_str(), 50);
	DrawText(text.c_str(), (int)rec.x + ((int)rec.width - length) / 2, (int)rec.y + (int)rec.height / 4, 50, Globals::Colors::BUTTON);
}

void Render()
{
	ClearBackground(Globals::Colors::BACKGROUND);
    int text_width = MeasureText("Chess", 150);
	DrawText("Chess", GetScreenWidth() / 2.0f - text_width / 2.0f, 200, 150, WHITE);
...
void OnResize(int width, int height)
{
    ...
    startRect = {width / 2.0f - 200, 600, 400, 120};
    loadRect = {width / 2.0f - 200, 750, 400, 120};
    settRect = {width / 2.0f - 200, 900, 400, 120};
    exitRect = {width / 2.0f - 200, 1050, 400, 120};
}
