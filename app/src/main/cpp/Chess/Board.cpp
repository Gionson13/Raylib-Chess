...
void SaveBoard(Board& board)
{
    ...
    oss << "Saves/" << std::put_time(&localtime, "%d-%m-%Y_%H-%M-%S") << ".csv";
...
//    outfile.close();
	...
	char* data = (char*)malloc(ss.str().size() + 1);
	strcpy(data, ss.str().c_str());
	bool success = SaveFileText(oss.str().c_str(), data);
	free(data);
	if (!success)
	{
		LOG_ERROR("Failed to save board");
	}
...
//        return false;
//    }
    char* file_data = LoadFileText(filename.c_str());
    if (!file_data)
    {
        LOG_WARN("Failed to open {}: {}", filename, std::strerror(errno));
        return false;
