...
namespace SettingsLayer
{
	...
	{
		int outerLength = MeasureText(outer.c_str(), 40);
		DrawText(outer.c_str(), rec.x - outerLength - 10, rec.y + rec.height / 4 , 40, WHITE);
		DrawRectangleLinesEx(rec, 3.0f, Globals::Colors::SETTINGS_BUTTON);
		int innerLength = MeasureText(inner.c_str(), 40);
		DrawText(inner.c_str(), (int)rec.x + ((int)rec.width - innerLength) / 2, (int)rec.y + (int)rec.height / 4, 40, Globals::Colors::SETTINGS_BUTTON);
	}
...
//			RenderSettingButton("", "Vsync: ", settingsVsync);
	...
	static void OnResize(int width, int height)
	{
		...
		settingsFps = {width / 2.0f - 50, height / 2.0f - 170, 270, 100};
		settingsRenderFPS = {width / 2.0f - 50, height / 2.0f - 50, 100, 100};
		settingsVsync = {width / 2.0f - 50, height / 2.0f + 70, 100, 100};
		settingClose = {width / 2.0f - 90, height / 2.0f + 200, 180, 100};
